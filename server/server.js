...
app.get("/graphQuickswap", async (req, res) => {

  console.log("Querying QuickSwap using The Graph...");
  const factories = await getInfo();
...
app.get("/price/:token", async (req, res) => {
  ...
  let price = Number(result.answer)/100000000;
  console.log(`The price of ` + token + ` in USD is ` + price);
...
app.get("/quickswap", async (req, res) => {
  ...
  console.log("Querying Quickswap...");
  
  //let result = await queryQuickswap();
